...
public class ShadowDOM extends BaseClass {
	  ...
	  public SearchContext expandRootElement (WebElement element) {
	   ...
	   public String getNestedTextUsingJS () {
		   WebElement shadowHost = getDriver().findElement (By.tagName("wc-product-tile"));
	       SearchContext shadowRoot = shadowHost.getShadowRoot ();
	       WebElement shadowContent = shadowRoot.findElement (By.cssSelector ("wc-add-to-cart.add-to-cart-button"));
	       SearchContext shadowRootTwo = shadowContent.getShadowRoot ();
	       String nestedText = shadowRootTwo.findElement (By.cssSelector ("button.add-to-cart-btn")).getText ();
	       return shadowHost.toString();
